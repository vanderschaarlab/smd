...
class LatentVarData(Dataset):
    ...
    def __init__(self, folder, scaling_factor, in_mem=True, verbose=True):
        ...
        self._in_mem = in_mem
        self._paths = [path for path in Path(f'{folder}').glob(f'**/*.pt')]
        ...
        if in_mem:
            ...
            progress = tqdm(range(0, total), desc='data loading') if verbose else range(0, total)
...
class Trainer(object):
    ...
    def __init__(
        ...
        *,
        ...
        if data_type == 'image':
            self.ds = ImageData(
                folder, self.image_size, in_mem=in_memory, augment_horizontal_flip=augment_horizontal_flip,
                convert_image_to=convert_image_to, verbose=self.accelerator.is_main_process
            )
        elif data_type == 'vec':
            self.ds = VecData(folder)
        else:
            self.ds = LatentVarData(folder, scaling_factor, in_mem=in_memory, verbose=self.accelerator.is_main_process)
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and divisible_by(self.step, self.save_and_sample_every):
                        ...
                        with torch.inference_mode():
                            ...
                            all_images_list = list(map(lambda n: self.ema.ema_model.sample(batch_size=n, clip_denoising=(self.data_type == 'image')), batches))
                        ...
                        if self.data_type == 'image':
                            ...
                            utils.save_image(mats, snapshot_path, nrow=int(math.sqrt(self.num_samples)))
