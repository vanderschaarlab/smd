...
class HyperNet(nn.Module):
    ...
    def __init__(self, layer_num, hidden_dim, latent_dim):
        ...
        assert layer_num >= 1
        self.param_projs = nn.ModuleList([nn.Linear(latent_dim, latent_dim) for _ in range(layer_num)])
    ...
    def get_weights(self, var):
        ...
        low_ranks = [torch.matmul(vec.unsqueeze(-1), vec.unsqueeze(-2)) for vec in params]
